...
function hideMessage() {
  el("appStatus").classList.add("hidden");
}
...
function parseRequestText(text) {
  ...
  const out = { case_number: "", container_number: "", chassis_number: "", location: "", repair_description: "" };
...
function handlePasteParse() {
  ...
  if (!text.trim()) {
    el("pasteStatus").textContent = "Paste text first.";
    return;
  }
  el("pasteStatus").textContent = "Parsing...";
  ...
  el("pasteStatus").textContent = found.length
...
function setForm(item = {}) {
  el("case_number").value = item.case_number || "";
  el("container_number").value = item.container_number || "";
  el("chassis_number").value = item.chassis_number || "";
  el("location").value = item.location || "";
  el("repair_description").value = item.repair_description || "";
  el("diag_ro_number").value = item.diag_ro_number || "";
  el("repair_ro_number").value = item.repair_ro_number || "";
  el("diag_comp_code").value = item.diag_comp_code || "";
  el("diag_cc_description").value = item.diag_cc_description || "";
  el("diag_cc_hours").value = item.diag_cc_hours || "";
  el("repair_comp_code").value = item.repair_comp_code || "";
  el("repair_cc_description").value = item.repair_cc_description || "";
  el("repair_cc_hours").value = item.repair_cc_hours || "";
  el("status").value = item.status || STATUS_OPTIONS[0];
  el("notes").value = item.notes || "";
}

function resetForm() {
  editingId = null;
  el("formTitle").textContent = "Add New Case";
  el("saveBtn").textContent = "Add Case";
  el("pasteInput").value = "";
  el("pasteStatus").textContent = "Waiting for pasted text.";
  setForm({});
}
...
function editItem(id) {
  ...
  if (!item) return;
  editingId = id;
  el("formTitle").textContent = "Edit Case";
...
function updateStats(items) {
  ...
  el("statPending").textContent = items.filter((x) => x.status === "Pending Shop Availability").length;
  el("statDiagOpen").textContent = items.filter((x) => x.status === "Diag RO Open").length;
  el("statCompleted").textContent = items.filter((x) => x.status === "Repair Order Completed").length;
}
...
function openModal(id) {
  ...
  el("m_diag_ro").value = item.diag_ro_number || "";
  el("m_diag_code").value = item.diag_comp_code || "";
  el("m_diag_desc").value = item.diag_cc_description || "";
  el("m_diag_hours").value = item.diag_cc_hours || "";
  el("m_rep_ro").value = item.repair_ro_number || "";
  el("m_rep_ref").value = item.diag_ro_number || "";
  el("m_rep_code").value = item.repair_comp_code || "";
  el("m_rep_desc").value = item.repair_cc_description || "";
  el("m_rep_hours").value = item.repair_cc_hours || "";
  el("m_rep_notes").value = item.notes || "";
...
function renderEmails() {
  ...
  if (!item) return;
  el("diagOut").textContent = buildDiagEmail(item);
  el("repOut").textContent = buildRepairEmail(item);
}
...
async function handleSession(session) {
  if (!session) {
    ...
    el("trackerView").classList.add("hidden");
    return;
  }
  el("authView").classList.add("hidden");
  el("trackerView").classList.remove("hidden");
  hideMessage();
  await loadRows();
  await setupRealtime();
}
...
async function init() {
  if (!HAS_CONFIG) {
    ...
    el("authView").classList.add("hidden");
    el("trackerView").classList.remove("hidden");
    showMessage("Update config.js and Supabase schema before deploying live.", "warning");
    return;
  }
...
function bindEvents() {
  ...
  ].forEach((id) => el(id).addEventListener("input", renderEmails));

  document.addEventListener("click", (e) => {
    const caseBtn = e.target.closest("[data-id]");
    if (caseBtn) { openModal(caseBtn.getAttribute("data-id")); return; }
    const editBtn = e.target.closest("[data-edit]");
    if (editBtn) { editItem(editBtn.getAttribute("data-edit")); return; }
    const nextBtn = e.target.closest("[data-next]");
    if (nextBtn) { nextStatus(nextBtn.getAttribute("data-next")); return; }
